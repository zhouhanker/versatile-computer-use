...
MCP（CU-D-280 tools/list；CU-D-290/300 CI live）：`vcu_hover` → `guide_hover`；`vcu_click` → `bm_click`；`vcu_session_abort` 拆 HUD。不要把 `vcu_session_stop` 当成 Escape abort。live `vcu_wait` 是 CU-D-310，尚未宣称。
