...
MCP（CU-D-280 tools/list；CU-D-290/300/310 CI live）：`vcu_hover` → `guide_hover`；`vcu_click` → `bm_click`；`vcu_wait` → `scene_wait`；`vcu_session_abort` 拆 HUD。不要把 `vcu_session_stop` 当成 Escape abort。live `vcu_type` 是 CU-D-320，尚未宣称。
