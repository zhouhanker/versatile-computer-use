...
MCP 工具：`vcu_browser_tabs` / `select` / `open` / `close` / `group` / `group_update` / `ungroup` / `screenshot` / `observe` / `click` / `hover` / `type` / `wait` / `scroll` / `key` / `ping` / `extract` / `login_state` / `install_lens`；桌面另有 `vcu_hover` / `vcu_session_abort` / `vcu_wait`（`value`）。CI 已 live hover/click/wait/type（无换行）/cmd 含换行 type 拒绝/abort；live scroll 尚未宣称。
