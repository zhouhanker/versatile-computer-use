# CU-WIN-SESSION-034: session type selects a named ListView row without moving the cursor.
# LVM_SETITEMSTATE, not SendInput. Not a WinForms ListBox.
...
public static class VcuCursor034 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor034]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-lv-034-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "VCU-LV-B" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "select failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "listview_select") { throw "path $($detail.input_path)" }
  ...
  $miss = (& $Vcu type --session $sid --ref $ref --text "VCU-LV-MISSING" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "missing row was selected" }
  $cursor1 = [VcuCursor034]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-034 OK $cid $sid ref=$ref path=listview_select value=VCU-LV-B cursor=$cursor1"
} finally {
