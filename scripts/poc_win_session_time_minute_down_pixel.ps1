# CU-WIN-SESSION-076: session clicks the minute field, then the down arrow.
# Minute field at 14 percent of the client width, then the down arrow. Not time_minute_pixel. Not DTM_SETSYSTEMTIME. Not SendInput.
...
public static class VcuCursor076 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor076]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-time-076-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
  ...
  '$dtp.ShowUpDown = $true',
  '$dtp.Value = Get-Date -Year 2026 -Month 1 -Day 2 -Hour 8 -Minute 1 -Second 0',
...
try {
  ...
  $cursorClick0 = [VcuCursor076]::Pos()
  $typed = (& $Vcu type --session $sid --ref $ref --text "timepix:mdn:08:00:00" | Out-String) | ConvertFrom-Json
  $cursorClick1 = [VcuCursor076]::Pos()
  if ($cursorClick0 -ne $cursorClick1) { throw "click moved cursor $cursorClick0 -> $cursorClick1" }
  if ($typed.ok -ne $true) { throw "time minute down pixel failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "time_minute_down_pixel") { throw "path $($detail.input_path)" }
  ...
  $cSame0 = [VcuCursor076]::Pos()
  $same = (& $Vcu type --session $sid --ref $ref --text "timepix:mdn:08:00:00" | Out-String) | ConvertFrom-Json
  $cSame1 = [VcuCursor076]::Pos()
  if ($cSame0 -ne $cSame1) { throw "same-time check moved cursor $cSame0 -> $cSame1" }
  if ($same.ok -eq $true) { throw "same time was accepted" }
  $bad = (& $Vcu type --session $sid --ref $ref --text "timepix:mdn:25:00:00" | Out-String) | ConvertFrom-Json
  if ($bad.ok -eq $true) { throw "invalid time was accepted" }
  $nope = (& $Vcu type --session $sid --ref $ref --text "timepix:mdn:nope" | Out-String) | ConvertFrom-Json
  if ($nope.ok -eq $true) { throw "non-time was accepted" }
  $skip = (& $Vcu type --session $sid --ref $ref --text "timepix:mdn:07:59:00" | Out-String) | ConvertFrom-Json
  if ($skip.ok -eq $true) { throw "hour wrap was accepted as one minute click" }
  $cursor1 = [VcuCursor076]::Pos()
  if ($cursorClick0 -ne $cursorClick1) { throw "click moved cursor $cursorClick0 -> $cursorClick1" }
  Write-Host "CU-WIN-SESSION-076 OK $cid $sid ref=$ref path=time_minute_down_pixel value=08:00:00 cursor=$cursorClick1"
} finally {
