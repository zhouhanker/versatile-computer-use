# CU-WIN-SESSION-057: session clicks a named tab header without moving the cursor.
# Mouse click on the tab rectangle. Not accSelect. Not TCM_SETCURSEL. Not SendInput. Cursor must not move.
...
public static class VcuCursor057 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor057]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-tpix-057-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "tabpix:VCU-TAB-B" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "tab pixel failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "tab_pixel") { throw "path $($detail.input_path)" }
  ...
  $again = (& $Vcu type --session $sid --ref $ref --text "tabpix:VCU-TAB-B" | Out-String) | ConvertFrom-Json
  if ($again.ok -eq $true) { throw "already selected tab was clicked" }
  $miss = (& $Vcu type --session $sid --ref $ref --text "tabpix:VCU-TAB-MISSING" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "missing tab was pixel-clicked" }
  $cursor1 = [VcuCursor057]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-057 OK $cid $sid ref=$ref path=tab_pixel value=VCU-TAB-B cursor=$cursor1"
} finally {
