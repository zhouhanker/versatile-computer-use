# CU-WIN-SESSION-066: session type unchecks a named list-view row without moving the cursor.
# Helper posts the mouse message because same-process UIA swallows it. Not LVM_SETITEMSTATE. Not SendInput. Cursor must not move.
...
public static class VcuCursor066 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor066]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-lvunpix-066-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
  'Add-Type -AssemblyName System.Windows.Forms | Out-Null',
  '$form = New-Object System.Windows.Forms.Form',
  '$form.Text = "VCU-LVUN-HOST"',
  '$form.TopMost = $true',
  ...
  '$label.Text = "VCU-LV-ON-B"',
  '$label.Bounds = New-Object System.Drawing.Rectangle 16, 12, 280, 28',
  '$lv.Add_ItemCheck({ if ($_.NewValue -eq [System.Windows.Forms.CheckState]::Unchecked -and $_.Index -eq 1) { $label.Text = "VCU-LV-OFF-B" } })',
...
try {
  ...
  $found = (& $Vcu wait --session $sid --name "VCU-LVUN-HOST" --ms 2500 | Out-String) | ConvertFrom-Json
  if ($found.ok -ne $true) { throw "list view host not found" }
  $ref = [string]$found.data.detail.found_ref
  $typed = (& $Vcu type --session $sid --ref $ref --text "lvuncheckpix:VCU-LV-B" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "lvuncheck pixel failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "lvuncheck_pixel") { throw "path $($detail.input_path)" }
  if ($detail.os_cursor_used -ne $false -or $detail.hid_injected -ne $false) { throw "cursor or hid used" }
  $label = (& $Vcu wait --session $sid --name "VCU-LV-OFF-B" --ms 2500 | Out-String) | ConvertFrom-Json
  if ($label.ok -ne $true) { throw "item uncheck event did not run" }
  $again = (& $Vcu type --session $sid --ref $ref --text "lvuncheckpix:VCU-LV-B" | Out-String) | ConvertFrom-Json
  if ($again.ok -eq $true) { throw "already unchecked was accepted" }
  $miss = (& $Vcu type --session $sid --ref $ref --text "lvuncheckpix:VCU-LV-MISSING" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "missing row was unchecked" }
  $cursor1 = [VcuCursor066]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-066 OK $cid $sid ref=$ref path=lvuncheck_pixel value=VCU-LV-B cursor=$cursor1"
} finally {
