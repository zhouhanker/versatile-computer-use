# WIN-VIS-009: Windows HUD uses live acrylic, with white text on a click-through overlay.
# Sampled blur remains the fallback. Not NSVisualEffectView. Does not move the OS cursor.
...
$modeFile = Join-Path $env:TEMP "vcu-vis-009-mode.txt"
...
$formScript = Join-Path $env:TEMP ("vcu-vis-009-" + [guid]::NewGuid().ToString("N") + ".ps1")
$formLines = @(
  'Add-Type -TypeDefinition @"',
  'using System;',
  'using System.Runtime.InteropServices;',
...
function Read-Gap {
  ...
  $g.CopyFromScreen(($x + [int]($w / 2)), ($y + [int]($h / 2)), 0, 0, (New-Object System.Drawing.Size 1, 1))
  $c = $bmp.GetPixel(0, 0)
...
try {
  ...
  Write-Host "WIN-VIS-009 OK acrylic green=$($green.R),$($green.G),$($green.B) red=$($red.R),$($red.G),$($red.B) bright=$bright cursor=$cursor1"
} finally {
