# CU-WIN-SESSION-058: session clicks a named row in an owned list box.
# Mouse click on the row rectangle. Not LB_SETCURSEL. Not SendInput. Cursor must not move.
...
public static class VcuCursor058 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor058]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-lpix-058-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
  'Add-Type -AssemblyName System.Windows.Forms | Out-Null',
  '$form = New-Object System.Windows.Forms.Form',
  '$form.Text = "VCU-LIST-HOST"',
  '$form.TopMost = $true',
  ...
  '$label.Text = "VCU-LIST-OLD"',
  '$label.Bounds = New-Object System.Drawing.Rectangle 24, 16, 280, 28',
  '$list.Add_SelectedIndexChanged({ if ($list.SelectedIndex -eq 1) { $label.Text = "VCU-LIST-HIT" } })',
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "listpix:VCU-LIST-B" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "list pixel failed $($typed | ConvertTo-Json -Compress -Depth 8)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "list_pixel") { throw "path $($detail.input_path)" }
  ...
  $hit = (& $Vcu wait --session $sid --name "VCU-LIST-HIT" --ms 2500 | Out-String) | ConvertFrom-Json
  if ($hit.ok -ne $true) { throw "selection event did not run" }
  $again = (& $Vcu type --session $sid --ref $ref --text "listpix:VCU-LIST-B" | Out-String) | ConvertFrom-Json
  if ($again.ok -eq $true) { throw "already selected row was clicked" }
  $miss = (& $Vcu type --session $sid --ref $ref --text "listpix:VCU-LIST-MISSING" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "missing row was pixel-clicked" }
  $farScript = Join-Path $env:TEMP ("vcu-lpix-058-far-" + [guid]::NewGuid().ToString("N") + ".ps1")
  $farLines = @(
    'Add-Type -AssemblyName System.Windows.Forms | Out-Null',
    '$form = New-Object System.Windows.Forms.Form',
    '$form.Text = "VCU-LIST-FAR-HOST"',
    '$form.TopMost = $true',
    '$form.StartPosition = "Manual"',
    '$form.Bounds = New-Object System.Drawing.Rectangle 140, 120, 420, 240',
    '$list = New-Object System.Windows.Forms.ListBox',
    ...
    '$list.Bounds = New-Object System.Drawing.Rectangle 24, 56, 220, 120',
    '$label = New-Object System.Windows.Forms.Label',
    '$label.Text = "VCU-LIST-FAR-OLD"',
    '$label.Bounds = New-Object System.Drawing.Rectangle 24, 16, 280, 28',
    '$list.Add_SelectedIndexChanged({ if ([string]$list.SelectedItem -eq "VCU-LIST-FAR") { $label.Text = "VCU-LIST-FAR-HIT" } })',
    '$form.Controls.Add($label)',
    '$form.Controls.Add($list)',
    '[System.Windows.Forms.Application]::Run($form)'
  )
  $farLines | Set-Content -Encoding ASCII -Path $farScript
  ...
  $ref = [string]$found.data.detail.found_ref
  $typed = (& $Vcu type --session $sid --ref $ref --text "listpix:VCU-LIST-FAR" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "far list pixel failed $($typed | ConvertTo-Json -Compress -Depth 8)" }
  if ($typed.data.detail.input_path -ne "list_pixel") { throw "far path $($typed.data.detail.input_path)" }
  $farHit = (& $Vcu wait --session $sid --name "VCU-LIST-FAR-HIT" --ms 2500 | Out-String) | ConvertFrom-Json
  if ($farHit.ok -ne $true) { throw "far selection event did not run" }
  Remove-Item $farScript -ErrorAction SilentlyContinue
  $cursor1 = [VcuCursor058]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-058 OK $cid $sid ref=$ref path=list_pixel selected=VCU-LIST-B far=VCU-LIST-FAR cursor=$cursor1"
} finally {
