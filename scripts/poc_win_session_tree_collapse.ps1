# CU-WIN-SESSION-037: session type collapses a named tree node without moving the cursor.
# TVM_EXPAND TVE_COLLAPSE. Not a pixel click. Not SendInput.
...
public static class VcuCursor037 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor037]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-col-037-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "collapse:VCU-COL-A" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "collapse failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "tree_collapse") { throw "path $($detail.input_path)" }
  ...
  if ($label.ok -ne $true) { throw "collapse event did not run" }
  $miss = (& $Vcu type --session $sid --ref $ref --text "collapse:VCU-COL-MISSING" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "missing node was collapsed" }
  $cursor1 = [VcuCursor037]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-037 OK $cid $sid ref=$ref path=tree_collapse value=VCU-COL-A cursor=$cursor1"
} finally {
