# CU-WIN-SESSION-010: clear all calculator memory, then recall must not restore 5.
...
function Press-Name($sid, $name) {
  ...
  if ($click.ok -ne $true) { throw "click failed: $name $($click | ConvertTo-Json -Compress)" }
...
try {
  ...
  Press-Name $sid "清除"
  Press-Name $sid "记忆调用"
  if ((Get-Display $sid) -ne "显示为 5") { throw "memory was not stored" }
  Press-Name $sid "清除所有记忆"
  Press-Name $sid "清除"
  if ((Get-Display $sid) -ne "显示为 0") { throw "display not cleared after MC: $(Get-Display $sid)" }
  $recall = $null
  $refs = Get-Refs $sid
  $btn = @($refs | Where-Object { $_.name -eq "记忆调用" -and $_.role -like "*Button*" } | Select-Object -First 1)
  if ($btn.Count -ne 1) {
    $recall = "button-missing"
  } else {
    $click = (& $Vcu click --session $sid --ref $btn[0].ref | Out-String) | ConvertFrom-Json
    if ($click.ok -ne $true) {
      $recall = "click-failed"
      ...
      $detail = $click.data.detail
      if ($detail.os_cursor_used -ne $false -or $detail.hid_injected -ne $false) { throw "cursor or hid used on recall" }
      if ($detail.input_path -ne "uia_invoke") { throw "path $($detail.input_path) on recall" }
      Start-Sleep -Milliseconds 280
      $recall = Get-Display $sid
    }
  }
  if ($recall -eq "显示为 5") { throw "clear-all memory did not stick: $recall" }
  if ($recall -ne "显示为 0" -and $recall -ne "click-failed" -and $recall -ne "button-missing") {
    throw "unexpected recall after MC: $recall"
  }
  ...
  Write-Host "CU-WIN-SESSION-010 OK $cid $sid MC then MR stays 0"
} finally {
