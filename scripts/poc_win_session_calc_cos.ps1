# CU-WIN-SESSION-015: scientific cos(0 degrees) = 1, then restore standard mode.
...
try {
  ...
  $zero = Press-Named $sid "零"
  if ($zero -ne "uia_invoke") { throw "zero path $zero" }
  $trig = Press-Named $sid "三角学"
  if ($trig -ne "toggle") { throw "trig path $trig" }
  $cos = Press-Named $sid "余弦"
  if ($cos -ne "uia_invoke") { throw "cos path $cos" }
  $shown = Get-Display $sid
  if ($shown -ne "显示为 1") { throw "cos display $shown" }
  ...
  Write-Host "CU-WIN-SESSION-015 OK $cid $sid cos(0)=1 restored"
} finally {
