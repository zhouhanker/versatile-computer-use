# CU-WIN-SESSION-067: session type selects a named ListView row without moving the cursor.
# Helper posts the mouse message on the row text. Not LVM_SETITEMSTATE. Not SendInput. Cursor must not move.
...
public static class VcuCursor067 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor067]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-lvrow-067-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
  'Add-Type -AssemblyName System.Windows.Forms | Out-Null',
  '$form = New-Object System.Windows.Forms.Form',
  '$form.Text = "VCU-LV-HOST"',
  '$form.TopMost = $true',
  ...
  '$label.Text = "VCU-LV-PICKED-A"',
  '$label.Bounds = New-Object System.Drawing.Rectangle 16, 12, 240, 28',
  '$lv.Add_ItemSelectionChanged({ if ($_.IsSelected) { $label.Text = "VCU-LV-PICKED-" + $_.Item.Text.Substring($_.Item.Text.Length - 1) } })',
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "lvrowpix:VCU-LV-B" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "row pixel failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "lvrow_pixel") { throw "path $($detail.input_path)" }
  if ($detail.os_cursor_used -ne $false -or $detail.hid_injected -ne $false) { throw "cursor or hid used" }
  $label = (& $Vcu wait --session $sid --name "VCU-LV-PICKED-B" --ms 2500 | Out-String) | ConvertFrom-Json
  if ($label.ok -ne $true) { throw "label did not follow selection" }
  $again = (& $Vcu type --session $sid --ref $ref --text "lvrowpix:VCU-LV-B" | Out-String) | ConvertFrom-Json
  if ($again.ok -eq $true) { throw "already selected was accepted" }
  ...
  $miss = (& $Vcu type --session $sid --ref $ref --text "lvrowpix:VCU-LV-MISSING" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "missing row was selected" }
  $cursor1 = [VcuCursor067]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-067 OK $cid $sid ref=$ref path=lvrow_pixel value=VCU-LV-B cursor=$cursor1"
} finally {
