# CU-WIN-SESSION-082: scientific square root of 9 is 3, then restore standard mode.
...
try {
  ...
  $nine = Press-Named $sid "九"
  if ($nine -ne "uia_invoke") { throw "digit path $nine" }
  if ((Get-Display $sid) -ne "显示为 9") { throw "not 9: $(Get-Display $sid)" }
  $root = Press-Named $sid "平方根"
  if ($root -ne "uia_invoke") { throw "sqrt path $root" }
  $shown = Get-Display $sid
  if ($shown -ne "显示为 3") { throw "sqrt display $shown" }
  ...
  $cursor1 = [VcuCursor082]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-082 OK $cid $sid sqrt(9)=3 restored cursor=$cursor1"
} finally {
