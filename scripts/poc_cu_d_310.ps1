# CU-D-310: MCP tools/call live vcu_wait value -> scene_wait on Notepad.
# ASCII-only. File stdin for MCP frames. No OS cursor warp or HID inject.
$ErrorActionPreference = "Stop"
if ($env:OS -ne "Windows_NT") {
  Write-Host "SKIP CU-D-310: not Windows_NT"
  exit 0
}
...
$ud = Join-Path $env:TEMP ("vcu-310-" + [guid]::NewGuid().ToString())
...
try {
  ...
  $initMsg = '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"vcu-310","version":"0"}}}'
  $waitMsg = '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"vcu_wait","arguments":{"session":"' + (Esc $sid) + '","ms":4000,"value":"' + (Esc $mark) + '","tab_id":"' + (Esc $tab) + '"}}}'
  ...
  if ($out -notlike "*scene_wait*") { throw "vcu_wait missing scene_wait stdout=[$out] stderr=[$err]" }
  if ($out -notlike "*found_ref*") { throw "vcu_wait missing found_ref stdout=[$out] stderr=[$err]" }
  if ($out -notlike "*os_cursor_used*") { throw "vcu_wait missing os_cursor_used stdout=[$out]" }
  Write-Host "WAIT_OK path=scene_wait via=mcp_tools_call"
  Write-Host "CU-D-310 OK"
} finally {
