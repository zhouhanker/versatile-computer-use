# CU-WIN-SESSION-014: scientific sin(30 degrees) = 0.5, then restore standard mode.
...
function Invoke-Sin30($sid) {
  Press-Named $sid "三" | Out-Null
  Press-Named $sid "零" | Out-Null
  $path = Press-Named $sid "三角学"
  if ($path -ne "toggle") { throw "trig path $path" }
  $sin = Press-Named $sid "正弦"
  if ($sin -ne "uia_invoke") { throw "sin path $sin" }
  return Get-Display $sid
}
...
try {
  ...
  $shown = Invoke-Sin30 $sid
  if ($shown -ne "显示为 0.5") {
    Press-Named $sid "清除" | Out-Null
    Press-Named $sid "度数切换" | Out-Null
    $shown = Invoke-Sin30 $sid
  }
  if ($shown -ne "显示为 0.5") { throw "sin display $shown" }
  Select-Mode $sid "标准 计算器" | Out-Null
  $sinBtn = @((Get-Refs $sid) | Where-Object { $_.name -eq "三角学" })
  if ($sinBtn.Count -ne 0) { throw "failed to restore standard mode" }
  ...
  Write-Host "CU-WIN-SESSION-014 OK $cid $sid sin(30)=0.5 restored"
} finally {
