# CU-WIN-SESSION-042: session type checks a named list-view row without moving the cursor.
# LVM_SETITEMSTATE state image. Not a pixel click. Not listview_select. Not SendInput.
...
public static class VcuCursor042 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor042]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-lvchk-042-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "lvcheck:VCU-LV-B" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "lvcheck failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "listview_check") { throw "path $($detail.input_path)" }
  ...
  $again = (& $Vcu type --session $sid --ref $ref --text "lvcheck:VCU-LV-B" | Out-String) | ConvertFrom-Json
  if ($again.ok -eq $true) { throw "already checked was accepted" }
  $miss = (& $Vcu type --session $sid --ref $ref --text "lvcheck:VCU-LV-MISSING" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "missing row was checked" }
  $cursor1 = [VcuCursor042]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-042 OK $cid $sid ref=$ref path=listview_check value=VCU-LV-B cursor=$cursor1"
} finally {
