# CU-WIN-SESSION-009: memory subtract 5-2, then recall 3. No OS cursor.
# Does not allowlist ApplicationFrameHost. Refuses if Calculator is already open.
...
try {
  ...
  Press-Name $sid "记忆减法"
  Press-Name $sid "清除"
  if ((Get-Display $sid) -ne "显示为 0") { throw "clear failed: $(Get-Display $sid)" }
  Press-Name $sid "记忆调用"
  $recalled = Get-Display $sid
  if ($recalled -ne "显示为 3") { throw "memory subtract recall $recalled" }
  ...
  Write-Host "CU-WIN-SESSION-009 OK $cid $sid M- 5-2 -> MR 3"
} finally {
