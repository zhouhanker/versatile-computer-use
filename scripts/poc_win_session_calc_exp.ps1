# CU-WIN-SESSION-088: scientific notation 2 exponent 3 is 2000, then restore standard mode.
...
try {
  ...
  $exp = Press-Named $sid "指数"
  if ($exp -ne "uia_invoke") { throw "exp path $exp" }
  $mid = Get-Display $sid
  if ($mid -ne "显示为 2.e+0") { throw "exp display $mid" }
  $three = Press-Named $sid "三"
  if ($three -ne "uia_invoke") { throw "digit3 path $three" }
  $after = Get-Display $sid
  if ($after -ne "显示为 2.e+3") { throw "exp digit display $after" }
  $eq = Press-Named $sid "等于"
  if ($eq -ne "uia_invoke") { throw "equals path $eq" }
  $shown = Get-Display $sid
  if ($shown -ne "显示为 2,000") { throw "exp result $shown" }
  ...
  $cursor1 = [VcuCursor088]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-088 OK $cid $sid 2,000 restored cursor=$cursor1"
} finally {
