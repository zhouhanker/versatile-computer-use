# CU-WIN-SESSION-023: same name on the window and the button still clicks the button.
...
public static class VcuCursor023 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor023]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-same-023-" + [guid]::NewGuid().ToString("N") + ".ps1")
$formLines = @(
  'Add-Type -AssemblyName System.Windows.Forms | Out-Null',
  'Add-Type -AssemblyName System.Drawing | Out-Null',
  '$form = New-Object System.Windows.Forms.Form',
  '$form.Text = "VCU-SAME-023"',
  '$form.FormBorderStyle = [System.Windows.Forms.FormBorderStyle]::FixedSingle',
  '$form.TopMost = $true',
  '$form.StartPosition = [System.Windows.Forms.FormStartPosition]::Manual',
  '$form.Bounds = New-Object System.Drawing.Rectangle 180, 200, 420, 200',
  '$btn = New-Object System.Windows.Forms.Button',
  '$btn.Text = "VCU-SAME-023"',
  ...
  '$btn.Add_Click({ $label.Text = "VCU-SAME-CLICKED" })',
...
try {
  ...
  $found = (& $Vcu wait --session $sid --name "VCU-SAME-023" --ms 2500 | Out-String) | ConvertFrom-Json
  if ($found.ok -ne $true) { throw "name not found" }
  $ref = [string]$found.data.detail.found_ref
  if ($ref -eq "e1") { throw "matched the window instead of the button" }
  ...
  $seen = (& $Vcu wait --session $sid --name "VCU-SAME-CLICKED" --ms 2500 | Out-String) | ConvertFrom-Json
  if ($seen.ok -ne $true) { throw "label did not change ref=$ref path=$($detail.input_path)" }
  $cursor1 = [VcuCursor023]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-023 OK $cid $sid ref=$ref path=$($detail.input_path) label=VCU-SAME-CLICKED cursor=$cursor1"
} finally {
