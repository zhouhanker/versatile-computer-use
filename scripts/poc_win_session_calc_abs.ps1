# CU-WIN-SESSION-085: scientific absolute value of -5 is 5, then restore standard mode.
...
try {
  ...
  $sign = Press-Named $sid "正负"
  if ($sign -ne "uia_invoke") { throw "sign path $sign" }
  $neg = Get-Display $sid
  if ($neg -eq "显示为 5") { throw "sign did not negate: $neg" }
  $abs = Press-Named $sid "绝对值"
  if ($abs -ne "uia_invoke") { throw "abs path $abs" }
  $shown = Get-Display $sid
  if ($shown -ne "显示为 5") { throw "abs display $shown neg=$neg" }
  ...
  $cursor1 = [VcuCursor085]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-085 OK $cid $sid abs(-5)=5 restored cursor=$cursor1"
} finally {
