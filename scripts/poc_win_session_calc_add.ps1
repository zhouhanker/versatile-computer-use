...
# CU-WIN-SESSION-003: session clicks 1 + 1 = through UIA. No OS cursor.
...
try {
  ...
  if ($before.Count -ne 1 -or $before[0].name -ne "显示为 0") { throw "display not zero" }
  Press-Name $sid "一"
  Press-Name $sid "加"
  Press-Name $sid "一"
  Press-Name $sid "等于"
  $after = @((Get-Refs $sid) | Where-Object { $_.name -like "显示为*" } | Select-Object -First 1)
  if ($after.Count -ne 1 -or $after[0].name -ne "显示为 2") { throw "display $($after[0].name)" }
  Write-Host "CU-WIN-SESSION-003 OK $cid $sid 1+1=2"
} finally {
