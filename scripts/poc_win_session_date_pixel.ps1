# CU-WIN-SESSION-068: session type sets an owned date picker without moving the cursor.
# Clicks the dropdown arrow, then a visible day in the open month. Not DTM_SETSYSTEMTIME. Not SendInput. Cursor must not move.
...
public static class VcuCursor068 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor068]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-dtpix-068-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "datepix:2026-01-20" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "date pixel failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "date_pixel") { throw "path $($detail.input_path)" }
  if ($detail.os_cursor_used -ne $false -or $detail.hid_injected -ne $false) { throw "cursor or hid used" }
  $label = (& $Vcu wait --session $sid --name "VCU-DTP-2026-01-20" --ms 2500 | Out-String) | ConvertFrom-Json
  if ($label.ok -ne $true) { throw "value changed event did not run" }
  $same = (& $Vcu type --session $sid --ref $ref --text "datepix:2026-01-20" | Out-String) | ConvertFrom-Json
  if ($same.ok -eq $true) { throw "same date was accepted" }
  $bad = (& $Vcu type --session $sid --ref $ref --text "datepix:2026-02-31" | Out-String) | ConvertFrom-Json
  if ($bad.ok -eq $true) { throw "invalid date was accepted" }
  $nope = (& $Vcu type --session $sid --ref $ref --text "datepix:nope" | Out-String) | ConvertFrom-Json
  if ($nope.ok -eq $true) { throw "non-date was accepted" }
  $other = (& $Vcu type --session $sid --ref $ref --text "datepix:2026-03-15" | Out-String) | ConvertFrom-Json
  if ($other.ok -eq $true) { throw "other month was accepted" }
  $cursor1 = [VcuCursor068]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-068 OK $cid $sid ref=$ref path=date_pixel value=2026-01-20 cursor=$cursor1"
} finally {
