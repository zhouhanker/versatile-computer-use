# CU-WIN-SESSION-087: scientific 2 to the power of 3 is 8, then restore standard mode.
...
try {
  ...
  $nine = Press-Named $sid "二"
  if ($nine -ne "uia_invoke") { throw "digit path $nine" }
  if ((Get-Display $sid) -ne "显示为 2") { throw "not 2: $(Get-Display $sid)" }
    $xyName = [string]([char]0x201C + 'X' + [char]0x201D + [char]0x7684 + [char]0x6307 + [char]0x6570)
  $xyRef = @((Get-Refs $sid) | Where-Object { $_.name -eq $xyName } | Select-Object -First 1)
  if ($xyRef.Count -ne 1) { throw "xy button missing" }
  $xyClick = (& $Vcu click --session $sid --ref $xyRef[0].ref | Out-String)
  $xyObj = $xyClick | ConvertFrom-Json
  $pow = [string]$xyObj.data.detail.input_path
  if ($pow -ne "uia_invoke" -and $pow -ne "xy_invoke") { throw "pow path $pow ref=$($xyRef[0].ref)" }
  Start-Sleep -Milliseconds 280
$mid = Get-Display $sid
  if ($mid -eq "显示为 8") { throw "power finished before the exponent: $mid" }
  $three = Press-Named $sid "三"
  if ($three -ne "uia_invoke") { throw "exponent path $three" }
  $eq = Press-Named $sid "等于"
  if ($eq -ne "uia_invoke") { throw "equals path $eq" }
  $shown = Get-Display $sid
  if ($shown -ne "显示为 8") { throw "pow display $shown mid=$mid" }
  ...
  $cursor1 = [VcuCursor087]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-087 OK $cid $sid 2^3=8 restored cursor=$cursor1"
} finally {
