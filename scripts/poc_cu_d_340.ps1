# CU-D-340: MCP tools/call live vcu_scroll on Notepad -> wm_vscroll.
# ASCII-only. File stdin for MCP frames. No OS cursor warp or HID inject.
$ErrorActionPreference = "Stop"
if ($env:OS -ne "Windows_NT") {
  Write-Host "SKIP CU-D-340: not Windows_NT"
  exit 0
}
...
$ud = Join-Path $env:TEMP ("vcu-340-" + [guid]::NewGuid().ToString())
...
try {
  ...
  $lines = (1..40 | ForEach-Object { "VCU-D-340 line $_" }) -join "`r`n"
  ...
  $initMsg = '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"vcu-340","version":"0"}}}'
  $scrollMsg = '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"vcu_scroll","arguments":{"session":"' + (Esc $sid) + '","tab_id":"' + (Esc $tab) + '","dy":600}}}'
  ...
  $path = $null
  if ($out -like "*wm_vscroll*") { $path = "wm_vscroll" }
  if (-not $path -and $out -like "*uia_scroll*") { $path = "uia_scroll" }
  if (-not $path) { throw "vcu_scroll missing wm_vscroll or uia_scroll stdout=[$out] stderr=[$err]" }
  if ($out -notlike "*os_cursor_used*") { throw "vcu_scroll missing os_cursor_used stdout=[$out]" }
  Write-Host ("SCROLL_OK path={0} os_cursor_used=False via=mcp_tools_call" -f $path)
  Write-Host "CU-D-340 OK"
} finally {
