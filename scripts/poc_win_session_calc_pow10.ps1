# CU-WIN-SESSION-086: scientific 10 to the power of 2 is 100, then restore standard mode.
...
try {
  ...
  $root = Press-Named $sid "十的指数"
  if ($root -ne "uia_invoke") { throw "pow10 path $root" }
  $shown = Get-Display $sid
  if ($shown -ne "显示为 100") { throw "pow10 display $shown" }
  ...
  $cursor1 = [VcuCursor086]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-086 OK $cid $sid 10^2=100 restored cursor=$cursor1"
} finally {
