# CU-WIN-SESSION-038: session type checks a named checked-list item without moving the cursor.
# LBC_SETCHECKSTATE. Not a pixel click. Not SendInput. Not list_select.
...
public static class VcuCursor038 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor038]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-chk-038-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "check:VCU-CHK-B" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "check failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "check_set") { throw "path $($detail.input_path)" }
  ...
  $miss = (& $Vcu type --session $sid --ref $ref --text "check:VCU-CHK-MISSING" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "missing item was checked" }
  $cursor1 = [VcuCursor038]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-038 OK $cid $sid ref=$ref path=check_set value=VCU-CHK-B cursor=$cursor1"
} finally {
