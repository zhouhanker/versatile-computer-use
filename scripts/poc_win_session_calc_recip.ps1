# CU-WIN-SESSION-084: scientific reciprocal of 4 is 0.25, then restore standard mode.
...
try {
  ...
  $nine = Press-Named $sid "四"
  if ($nine -ne "uia_invoke") { throw "digit path $nine" }
  if ((Get-Display $sid) -ne "显示为 4") { throw "not 4: $(Get-Display $sid)" }
  $root = Press-Named $sid "倒数"
  if ($root -ne "uia_invoke") { throw "reciprocal path $root" }
  $shown = Get-Display $sid
  if ($shown -ne "显示为 0.25") { throw "reciprocal display $shown" }
  ...
  $cursor1 = [VcuCursor084]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-084 OK $cid $sid reciprocal(4)=0.25 restored cursor=$cursor1"
} finally {
