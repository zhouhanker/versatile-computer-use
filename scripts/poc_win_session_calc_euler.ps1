# CU-WIN-SESSION-089: scientific Euler number, then restore standard mode.
...
try {
  ...
  $euler = Press-Named $sid "Euler 数字"
  if ($euler -ne "uia_invoke") { throw "euler path $euler" }
  $shown = Get-Display $sid
  if (-not $shown.StartsWith("显示为 2.718281828")) { throw "euler display $shown" }
  ...
  $cursor1 = [VcuCursor089]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-089 OK $cid $sid e restored cursor=$cursor1"
} finally {
