# CU-WIN-SESSION-083: scientific square of 8 is 64, then restore standard mode.
...
try {
  ...
  $root = Press-Named $sid "平方"
  if ($root -ne "uia_invoke") { throw "square path $root" }
  $shown = Get-Display $sid
  if ($shown -ne "显示为 64") { throw "square display $shown" }
  ...
  $cursor1 = [VcuCursor083]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-083 OK $cid $sid square(8)=64 restored cursor=$cursor1"
} finally {
