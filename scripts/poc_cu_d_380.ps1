# CU-D-380: MCP tools/call live vcu_wait miss times out honestly.
# ASCII-only. File stdin for MCP frames. No OS cursor warp or HID inject.
$ErrorActionPreference = "Stop"
if ($env:OS -ne "Windows_NT") {
  Write-Host "SKIP CU-D-380: not Windows_NT"
  exit 0
}
...
$ud = Join-Path $env:TEMP ("vcu-380-" + [guid]::NewGuid().ToString())
...
try {
  ...
  $initMsg = '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"vcu-380","version":"0"}}}'
  $missMsg = '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"vcu_wait","arguments":{"session":"' + (Esc $sid) + '","tab_id":"' + (Esc $tab) + '","ms":800,"value":"VCU-D-380-MISS"}}}'
  $refMsg = '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"vcu_wait","arguments":{"session":"' + (Esc $sid) + '","tab_id":"' + (Esc $tab) + '","ms":800,"ref":"e999"}}}'
  $abortMsg = '{"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"vcu_session_abort","arguments":{"session":"' + (Esc $sid) + '"}}}'

  $ms = New-Object System.IO.MemoryStream
  Add-McpFrame $ms $initMsg
  Add-McpFrame $ms $missMsg
  Add-McpFrame $ms $refMsg
  ...
  if ($out -notlike "*ActionFailed*") { throw "wait miss missing ActionFailed stdout=[$(Clip $out)] stderr=[$(Clip $err)]" }
  if ($out -notlike "*timed out*") { throw "wait miss missing timed out stdout=[$(Clip $out)] stderr=[$(Clip $err)]" }
  if ($out -notlike "*VCU-D-380-MISS*") { throw "wait miss missing value stdout=[$(Clip $out)] stderr=[$(Clip $err)]" }
  if ($out -notlike "*e999*") { throw "wait ref miss missing e999 stdout=[$(Clip $out)] stderr=[$(Clip $err)]" }
  Write-Host "WAIT_MISS_OK via=mcp_tools_call"
  Write-Host "WAIT_REF_MISS_OK via=mcp_tools_call"
  Write-Host "CU-D-380 OK"
} finally {
