# CU-WIN-SESSION-033: session type selects a named tree node without moving the cursor.
# TVM_SELECTITEM, not SendInput.
...
public static class VcuCursor033 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor033]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-tree-033-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "VCU-TREE-B" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "select failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "tree_select") { throw "path $($detail.input_path)" }
  ...
  $miss = (& $Vcu type --session $sid --ref $ref --text "VCU-TREE-MISSING" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "missing node was selected" }
  $cursor1 = [VcuCursor033]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-033 OK $cid $sid ref=$ref path=tree_select value=VCU-TREE-B cursor=$cursor1"
} finally {
