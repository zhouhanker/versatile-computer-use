# CU-WIN-SESSION-040: session type sets an owned date picker without moving the cursor.
# DTM_SETSYSTEMTIME plus reflected DTN_DATETIMECHANGE. Not a pixel click. Not SendInput.
...
public static class VcuCursor040 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor040]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-dtp-040-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "date:2026-03-15" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "date failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "date_set") { throw "path $($detail.input_path)" }
  ...
  $same = (& $Vcu type --session $sid --ref $ref --text "date:2026-03-15" | Out-String) | ConvertFrom-Json
  if ($same.ok -eq $true) { throw "same date was accepted" }
  $bad = (& $Vcu type --session $sid --ref $ref --text "date:2026-02-31" | Out-String) | ConvertFrom-Json
  if ($bad.ok -eq $true) { throw "invalid date was accepted" }
  $nope = (& $Vcu type --session $sid --ref $ref --text "date:nope" | Out-String) | ConvertFrom-Json
  if ($nope.ok -eq $true) { throw "non-date was accepted" }
  $cursor1 = [VcuCursor040]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-040 OK $cid $sid ref=$ref path=date_set value=2026-03-15 cursor=$cursor1"
} finally {
