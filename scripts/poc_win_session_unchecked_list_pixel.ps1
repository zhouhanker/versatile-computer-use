# CU-WIN-SESSION-062: session clicks the checkbox to clear a named checked-list row.
...
public static class VcuCursor062 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor062]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-unchkpix-062-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
  'Add-Type -AssemblyName System.Windows.Forms | Out-Null',
  '$form = New-Object System.Windows.Forms.Form',
  '$form.Text = "VCU-UNCHK-HOST"',
  '$form.TopMost = $true',
  ...
  '$label.Text = "VCU-CHK-ON-B"',
  '$label.Bounds = New-Object System.Drawing.Rectangle 16, 12, 280, 28',
  '$list.Add_ItemCheck({ if ($_.NewValue -eq [System.Windows.Forms.CheckState]::Unchecked -and $_.Index -eq 1) { $label.Text = "VCU-CHK-OFF-B" } })',
...
try {
  ...
  $found = (& $Vcu wait --session $sid --name "VCU-UNCHK-HOST" --ms 2500 | Out-String) | ConvertFrom-Json
  if ($found.ok -ne $true) { throw "list not found" }
  $ref = [string]$found.data.detail.found_ref
  $typed = (& $Vcu type --session $sid --ref $ref --text "uncheckpix:VCU-CHK-B" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "uncheck pixel failed $($typed | ConvertTo-Json -Compress -Depth 8)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "uncheck_pixel") { throw "path $($detail.input_path)" }
  if ($detail.os_cursor_used -ne $false -or $detail.hid_injected -ne $false) { throw "cursor or hid used" }
  $label = (& $Vcu wait --session $sid --name "VCU-CHK-OFF-B" --ms 2500 | Out-String) | ConvertFrom-Json
  if ($label.ok -ne $true) { throw "item uncheck event did not run" }
  $again = (& $Vcu type --session $sid --ref $ref --text "uncheckpix:VCU-CHK-B" | Out-String) | ConvertFrom-Json
  if ($again.ok -eq $true) { throw "already unchecked row was clicked" }
  $miss = (& $Vcu type --session $sid --ref $ref --text "uncheckpix:VCU-CHK-MISSING" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "missing row was pixel-unchecked" }
  $cursor1 = [VcuCursor062]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-062 OK $cid $sid ref=$ref path=uncheck_pixel value=VCU-CHK-B cursor=$cursor1"
} finally {
