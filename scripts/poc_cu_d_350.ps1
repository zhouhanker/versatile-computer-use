# CU-D-350: MCP tools/call live vcu_extract reads back Notepad Scene value.
# ASCII-only. File stdin for MCP frames. No OS cursor warp or HID inject.
$ErrorActionPreference = "Stop"
if ($env:OS -ne "Windows_NT") {
  Write-Host "SKIP CU-D-350: not Windows_NT"
  exit 0
}
...
$ud = Join-Path $env:TEMP ("vcu-350-" + [guid]::NewGuid().ToString())
...
try {
  ...
  $initMsg = '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"vcu-350","version":"0"}}}'
  $exMsg = '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"vcu_extract","arguments":{"session":"' + (Esc $sid) + '","tab_id":"' + (Esc $tab) + '","selector":"' + (Esc $mark) + '"}}}'
