# CU-WIN-SESSION-077: session clicks the second field, then the down arrow.
# Second field at 26 percent of the client width, then the down arrow. Not time_second_pixel. Not DTM_SETSYSTEMTIME. Not SendInput.
...
public static class VcuCursor077 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor077]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-time-077-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
  ...
  '$dtp.ShowUpDown = $true',
  '$dtp.Value = Get-Date -Year 2026 -Month 1 -Day 2 -Hour 8 -Minute 0 -Second 1',
...
try {
  ...
  $cursorClick0 = [VcuCursor077]::Pos()
  $typed = (& $Vcu type --session $sid --ref $ref --text "timepix:sdn:08:00:00" | Out-String) | ConvertFrom-Json
  $cursorClick1 = [VcuCursor077]::Pos()
  if ($cursorClick0 -ne $cursorClick1) { throw "click moved cursor $cursorClick0 -> $cursorClick1" }
  if ($typed.ok -ne $true) { throw "time second down pixel failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "time_second_down_pixel") { throw "path $($detail.input_path)" }
  if ($detail.os_cursor_used -ne $false -or $detail.hid_injected -ne $false) { throw "cursor or hid used" }
  $label = (& $Vcu wait --session $sid --name "VCU-TIME-08:00:00" --ms 2500 | Out-String) | ConvertFrom-Json
  if ($label.ok -ne $true) { throw "value changed event did not run" }
  $cSame0 = [VcuCursor077]::Pos()
  $same = (& $Vcu type --session $sid --ref $ref --text "timepix:sdn:08:00:00" | Out-String) | ConvertFrom-Json
  $cSame1 = [VcuCursor077]::Pos()
  if ($cSame0 -ne $cSame1) { throw "same-time check moved cursor $cSame0 -> $cSame1" }
  if ($same.ok -eq $true) { throw "same time was accepted" }
  $bad = (& $Vcu type --session $sid --ref $ref --text "timepix:sdn:25:00:00" | Out-String) | ConvertFrom-Json
  if ($bad.ok -eq $true) { throw "invalid time was accepted" }
  $nope = (& $Vcu type --session $sid --ref $ref --text "timepix:sdn:nope" | Out-String) | ConvertFrom-Json
  if ($nope.ok -eq $true) { throw "non-time was accepted" }
  $skip = (& $Vcu type --session $sid --ref $ref --text "timepix:sdn:07:59:59" | Out-String) | ConvertFrom-Json
  if ($skip.ok -eq $true) { throw "minute wrap was accepted as one second click" }
  $cursor1 = [VcuCursor077]::Pos()
  if ($cursorClick0 -ne $cursorClick1) { throw "click moved cursor $cursorClick0 -> $cursorClick1" }
  Write-Host "CU-WIN-SESSION-077 OK $cid $sid ref=$ref path=time_second_down_pixel value=08:00:00 cursor=$cursorClick1"
} finally {
