# CU-WIN-SESSION-072: session clicks the up arrow of an owned time picker.
# ShowUpDown child, top half. Not time_set. Not DTM_SETSYSTEMTIME. Not SendInput.
...
public static class VcuCursor072 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor072]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-time-072-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "timepix:up:09:00:00" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "time pixel failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "time_pixel") { throw "path $($detail.input_path)" }
  if ($detail.os_cursor_used -ne $false -or $detail.hid_injected -ne $false) { throw "cursor or hid used" }
  $label = (& $Vcu wait --session $sid --name "VCU-TIME-09:00:00" --ms 2500 | Out-String) | ConvertFrom-Json
  if ($label.ok -ne $true) { throw "value changed event did not run" }
  $same = (& $Vcu type --session $sid --ref $ref --text "timepix:up:09:00:00" | Out-String) | ConvertFrom-Json
  if ($same.ok -eq $true) { throw "same time was accepted" }
  $bad = (& $Vcu type --session $sid --ref $ref --text "timepix:up:25:00:00" | Out-String) | ConvertFrom-Json
  if ($bad.ok -eq $true) { throw "invalid time was accepted" }
  $nope = (& $Vcu type --session $sid --ref $ref --text "timepix:up:nope" | Out-String) | ConvertFrom-Json
  if ($nope.ok -eq $true) { throw "non-time was accepted" }
  $skip = (& $Vcu type --session $sid --ref $ref --text "timepix:up:11:00:00" | Out-String) | ConvertFrom-Json
  if ($skip.ok -eq $true) { throw "two-hour jump was accepted as one click" }
  $cursor1 = [VcuCursor072]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-072 OK $cid $sid ref=$ref path=time_pixel value=09:00:00 cursor=$cursor1"
} finally {
