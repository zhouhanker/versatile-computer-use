# CU-D-120: vcu desktop open_path opens a throwaway folder in Explorer (no SendInput).
$ErrorActionPreference = "Stop"
if ($env:OS -ne "Windows_NT") {
  Write-Host "SKIP CU-D-120: not Windows_NT"
  exit 0
}
...
$ud = Join-Path $env:TEMP ("vcu-120-" + [guid]::NewGuid().ToString())
...
try {
  ...
  if (-not $acted.ok) { throw "open_path failed" }
  ...
  if ($pathName -ne "explorer_open") { throw "unexpected input_path" }
  Write-Host ("OPEN_OK path={0} os_cursor_used={1}" -f $pathName, $cursor)

  & $vcu --user-dir $ud session abort $sid | Out-Null
  Write-Host "CU-D-120 OK"
} finally {
