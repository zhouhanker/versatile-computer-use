# CU-D-320: MCP tools/call live vcu_type without newline on Notepad.
# ASCII-only. File stdin for MCP frames. No OS cursor warp or HID inject.
$ErrorActionPreference = "Stop"
if ($env:OS -ne "Windows_NT") {
  Write-Host "SKIP CU-D-320: not Windows_NT"
  exit 0
}
...
$ud = Join-Path $env:TEMP ("vcu-320-" + [guid]::NewGuid().ToString())
...
try {
  ...
  $mark = "VCU-D-320-MARK"
  $initMsg = '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"vcu-320","version":"0"}}}'
  $typeMsg = '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"vcu_type","arguments":{"session":"' + (Esc $sid) + '","ref":"' + (Esc $ref) + '","tab_id":"' + (Esc $tab) + '","text":"' + (Esc $mark) + '"}}}'
  ...
  $path = $null
  if ($out -like "*wm_settext*") { $path = "wm_settext" }
  if ($out -like "*clipboard_paste*") { $path = "clipboard_paste" }
  if (-not $path) { throw "vcu_type missing wm_settext or clipboard_paste stdout=[$out] stderr=[$err]" }
  if ($out -notlike "*os_cursor_used*") { throw "vcu_type missing os_cursor_used stdout=[$out]" }
  Write-Host ("TYPE_OK path={0} os_cursor_used=False via=mcp_tools_call" -f $path)
  Write-Host "CU-D-320 OK"
} finally {
