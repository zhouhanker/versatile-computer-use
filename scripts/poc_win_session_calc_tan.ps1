# CU-WIN-SESSION-016: scientific tan(45 degrees) = 1, then restore standard mode.
...
try {
  ...
  $four = Press-Named $sid "四"
  if ($four -ne "uia_invoke") { throw "digit path $four" }
  Press-Named $sid "五" | Out-Null
  if ((Get-Display $sid) -ne "显示为 45") { throw "not 45: $(Get-Display $sid)" }
  $trig = Press-Named $sid "三角学"
  if ($trig -ne "toggle") { throw "trig path $trig" }
  $tan = Press-Named $sid "正切"
  if ($tan -ne "uia_invoke") { throw "tan path $tan" }
  $shown = Get-Display $sid
  if ($shown -ne "显示为 1") { throw "tan display $shown" }
  ...
  Write-Host "CU-WIN-SESSION-016 OK $cid $sid tan(45)=1 restored"
} finally {
