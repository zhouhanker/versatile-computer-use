# CU-WIN-SESSION-008: memory add 2+3, then recall 5. No OS cursor.
...
try {
  ...
  if ($recalled -ne "显示为 5") { throw "memory add recall $recalled" }
  ...
  Write-Host "CU-WIN-SESSION-008 OK $cid $sid M+ 2+3 -> MR 5"
} finally {
