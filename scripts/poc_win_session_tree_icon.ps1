# CU-WIN-SESSION-053: session clicks the expand icon of a named tree node.
# Mouse messages on the expand icon. Not TVM_EXPAND. Not SendInput. Cursor must not move.
...
public static class VcuCursor053 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor053]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-icon-053-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "treeicon:VCU-EXP-A" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "tree icon failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "tree_icon") { throw "path $($detail.input_path)" }
  ...
  if ($label.ok -ne $true) { throw "icon expand event did not run" }
  $miss = (& $Vcu type --session $sid --ref $ref --text "treeicon:VCU-EXP-MISSING" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "missing node was icon-expanded" }
  $again = (& $Vcu type --session $sid --ref $ref --text "treeicon:VCU-EXP-A" | Out-String) | ConvertFrom-Json
  if ($again.ok -eq $true) { throw "already expanded icon click was accepted" }
  $cursor1 = [VcuCursor053]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-053 OK $cid $sid ref=$ref path=tree_icon value=VCU-EXP-A cursor=$cursor1"
} finally {
