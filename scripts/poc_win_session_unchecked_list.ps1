# CU-WIN-SESSION-039: session type unchecks a named checked-list item without moving the cursor.
# LBC_SETCHECKSTATE 0. Not a pixel click. Not SendInput. Not list_select.
...
public static class VcuCursor039 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor039]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-unchk-039-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
...
try {
  ...
  if ($found.ok -ne $true) { throw "list host not found" }
  $ref = [string]$found.data.detail.found_ref
  $typed = (& $Vcu type --session $sid --ref $ref --text "uncheck:VCU-CHK-B" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "uncheck failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "uncheck_set") { throw "path $($detail.input_path)" }
  ...
  $miss = (& $Vcu type --session $sid --ref $ref --text "uncheck:VCU-CHK-MISSING" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "missing item was unchecked" }
  $cursor1 = [VcuCursor039]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-039 OK $cid $sid ref=$ref path=uncheck_set value=VCU-CHK-B cursor=$cursor1"
} finally {
