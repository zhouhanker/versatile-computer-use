# CU-WIN-SESSION-046: session type unchecks a named list-view row without moving the cursor.
# LVM_SETITEMSTATE state image 1. Not a pixel click. Not listview_check. Not SendInput.
...
public static class VcuCursor046 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor046]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-lvun-046-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "lvuncheck:VCU-LV-B" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "lvuncheck failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "listview_uncheck") { throw "path $($detail.input_path)" }
  ...
  $again = (& $Vcu type --session $sid --ref $ref --text "lvuncheck:VCU-LV-B" | Out-String) | ConvertFrom-Json
  if ($again.ok -eq $true) { throw "already unchecked was accepted" }
  $miss = (& $Vcu type --session $sid --ref $ref --text "lvuncheck:VCU-LV-MISSING" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "missing row was unchecked" }
  $cursor1 = [VcuCursor046]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-046 OK $cid $sid ref=$ref path=listview_uncheck value=VCU-LV-B cursor=$cursor1"
} finally {
