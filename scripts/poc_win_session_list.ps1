# CU-WIN-SESSION-030: session type selects a named row in an owned list box.
# LB_SETCURSEL plus reflected selection change. No SendInput. Does not move the OS cursor.
...
public static class VcuCursor030 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor030]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-list-030-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
  ...
  '$list.Bounds = New-Object System.Drawing.Rectangle 24, 24, 220, 120',
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "VCU-LIST-B" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "select failed" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "list_select") { throw "path $($detail.input_path)" }
  ...
  $miss = (& $Vcu type --session $sid --ref $ref --text "VCU-LIST-MISSING" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "missing row was selected" }
  $cursor1 = [VcuCursor030]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-030 OK $cid $sid ref=$ref path=list_select selected=VCU-LIST-B cursor=$cursor1"
} finally {
