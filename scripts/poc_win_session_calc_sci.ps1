# CU-WIN-SESSION-011: switch Calculator to scientific mode, insert pi, then restore standard.
...
try {
  ...
  $pi = @((Get-Refs $sid) | Where-Object { $_.name -eq "π" })
  if ($pi.Count -ne 0) { throw "pi still present in standard mode" }
  Select-Mode $sid "科学 计算器"
  $mode = @((Get-Refs $sid) | Where-Object { $_.name -eq "科学 计算器模式" } | Select-Object -First 1)
  if ($mode.Count -ne 1) { throw "scientific mode label missing" }
  $piPath = Press-Named $sid "π"
  if ($piPath -ne "uia_invoke") { throw "pi path $piPath" }
  $shown = Get-Display $sid
  if ($shown -notlike "显示为 3.1415926535*") { throw "pi display $shown" }
  Select-Mode $sid "标准 计算器"
  $pi = @((Get-Refs $sid) | Where-Object { $_.name -eq "π" })
  if ($pi.Count -ne 0) { throw "failed to restore standard mode" }
  ...
  Write-Host "CU-WIN-SESSION-011 OK $cid $sid scientific $shown restored"
} finally {
