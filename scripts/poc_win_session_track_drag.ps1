# CU-WIN-SESSION-056: session drags an owned trackbar thumb without moving the cursor.
# Mouse down, move, and up on the thumb. Not TBM_SETPOS. Not SendInput. Cursor must not move.
...
public static class VcuCursor056 {
  [StructLayout(LayoutKind.Sequential)] public struct POINT { public int x; public int y; }
  [DllImport("user32.dll")] public static extern bool GetCursorPos(out POINT p);
  public static string Pos() { POINT p; GetCursorPos(out p); return p.x.ToString() + "," + p.y.ToString(); }
}
"@
$cursor0 = [VcuCursor056]::Pos()
$formScript = Join-Path $env:TEMP ("vcu-tdrag-056-" + [guid]::NewGuid().ToString("N") + ".ps1")
@(
...
try {
  ...
  $typed = (& $Vcu type --session $sid --ref $ref --text "trackdrag:40" | Out-String) | ConvertFrom-Json
  if ($typed.ok -ne $true) { throw "drag failed $($typed.error)" }
  $detail = $typed.data.detail
  if ($detail.input_path -ne "track_drag") { throw "path $($detail.input_path)" }
  ...
  $again = (& $Vcu type --session $sid --ref $ref --text "trackdrag:40" | Out-String) | ConvertFrom-Json
  if ($again.ok -eq $true) { throw "same position was dragged again" }
  ...
  $miss = (& $Vcu type --session $sid --ref $ref --text "trackdrag:400" | Out-String) | ConvertFrom-Json
  if ($miss.ok -eq $true) { throw "out of range was accepted" }
  $bad = (& $Vcu type --session $sid --ref $ref --text "trackdrag:nope" | Out-String) | ConvertFrom-Json
  if ($bad.ok -eq $true) { throw "non-numeric was accepted" }
  $cursor1 = [VcuCursor056]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-056 OK $cid $sid ref=$ref path=track_drag value=40 cursor=$cursor1"
} finally {
