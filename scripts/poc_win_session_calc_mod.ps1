# CU-WIN-SESSION-090: scientific 8 modulo 3 is 2, then restore standard mode.
...
try {
  ...
  $nine = Press-Named $sid "八"
  if ($nine -ne "uia_invoke") { throw "digit path $nine" }
  if ((Get-Display $sid) -ne "显示为 8") { throw "not 8: $(Get-Display $sid)" }
    $mod = Press-Named $sid "模数"
  if ($mod -ne "uia_invoke") { throw "mod path $mod" }
  $mid = Get-Display $sid
  if ($mid -eq "显示为 2") { throw "modulo finished before the divisor: $mid" }
  $three = Press-Named $sid "三"
  if ($three -ne "uia_invoke") { throw "divisor path $three" }
  $eq = Press-Named $sid "等于"
  if ($eq -ne "uia_invoke") { throw "equals path $eq" }
  $shown = Get-Display $sid
  if ($shown -ne "显示为 2") { throw "mod display $shown mid=$mid" }
  ...
  $cursor1 = [VcuCursor090]::Pos()
  if ($cursor0 -ne $cursor1) { throw "cursor moved $cursor0 -> $cursor1" }
  Write-Host "CU-WIN-SESSION-090 OK $cid $sid 8mod3=2 restored cursor=$cursor1"
} finally {
