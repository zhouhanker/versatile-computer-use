# CU-D-100: vcu desktop InvokePattern on a throwaway WinForms button (no SendInput).
$ErrorActionPreference = "Stop"
if ($env:OS -ne "Windows_NT") {
  Write-Host "SKIP CU-D-100: not Windows_NT"
  exit 0
}
...
$ud = Join-Path $env:TEMP ("vcu-100-" + [guid]::NewGuid().ToString())
...
try {
  ...
  foreach ($r in @($snap.data.dom_refs)) {
    $nm = [string]$r.name
    if ($nm -eq "VcuCount") { $ref = [string]$r.ref; break }
  }
