...
$ud = Join-Path $env:TEMP ("vcu-090-" + [guid]::NewGuid().ToString())
...
try {
  ...
  if ($src -ne "uia_scene") { throw "source expected uia_scene" }
  $ref = $null
  foreach ($r in @($snap.data.dom_refs)) {
    $role = [string]$r.role
    if ($role -like "*Edit*") { $ref = [string]$r.ref; break }
  }
  if (-not $ref) { throw "no Edit ref in snapshot" }
  Write-Host ("SNAP_OK source={0} ref={1}" -f $src, $ref)

  $typeRaw = & $vcu --user-dir $ud type --session $sid --tab $tab --ref $ref --text "VCU-D-090" --json
  Write-Host $typeRaw
