...
| ID | 工作 | 验收 |
| --- | --- | --- |
| CU-D-060 | UIA 列窗 / 截图 / Invoke | **脚本+解析单测** 列窗；UIA 树/`InvokePattern` 脚本（无 SendInput）；Windows 上 snapshot/invoke 走 UIA。**真机未跑**（本机 macOS）。截图仍未接 PrintWindow。不得写成产品已有 Windows CU。 |
