...
- 时间框上箭头是 `time_pixel`，下箭头是 `time_down_pixel`。先点分钟字段再点上箭头是 `time_minute_pixel`。先点秒字段再点上箭头是 `time_second_pixel`。分钟字段的下箭头是 `time_minute_down_pixel`。秒字段的下箭头不在本切片。数字框微调仍是 `spin_up` / `spin_down`。跨进程改文字如果没有对应变更事件，不能报成功。
