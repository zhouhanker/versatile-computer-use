...
- [ ] **CU-D-520**：双浏览器 `tabs` 同一次响应含 Edge。单测：无 client_id 的 Edge poll 仍能接到定向 list_tabs；一侧失败不丢另一侧。真机仍常 Chrome-only（Edge hello≠poll）。不是完成。无 SendInput。
