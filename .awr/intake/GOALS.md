...
当前收尾（2026-09-25）：浏览器 0.2.8 与 CU-D-700 发布托管已完成。CI run `36129175359` 已恢复。可领取项只剩停放的 MAC-NEXT 与 FEISHU-001，未点名不开。Windows 真机 WIN-FIX-001 至 007 的已复测范围见 docs/PLAN-WINDOWS-FIX.md。007 只解决 POC 解码崩溃，不是完整 Windows 产品 CU。下一步大纲在 `docs/PLAN.md`，默认不新开切片。保留 1/3 标签组。用户已重置总额度，旧不足10%提示无效。
